...
export const authorizeAction = (action: string) => {
  return async (
    ...
    try {
      ...
      const principalRole = req.user?.role; // Retrieve the user role
      ...
      if (!principalId || !principalRole || !token) {
        return res
          .status(401)
          .json({ message: "Unauthorized. User or token missing." });
      }
      ...
      console.log("Policy Service Url >>>>>>>>>", config.policyServiceUrl);

      const response = await axios.post(
        `${config.policyServiceUrl}/v1/policies/evaluate`,
        policyPayload,
        {
          headers: {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response) {
          ...
          console.error(
            "No response received from Policy Service:",
            error.request
          );
          return res
            .status(500)
            .json({ message: "No response received from Policy Service" });
        } else {
          // Error setting up the request
          console.error(
            "Error setting up request to Policy Service:",
            error.message
          );
          return res
            .status(500)
            .json({ message: "Error setting up request to Policy Service" });
        }
      } else {
        ...
        return res
          .status(500)
          .json({ message: "Internal server error during policy evaluation." });
      }
    }
  };
};
