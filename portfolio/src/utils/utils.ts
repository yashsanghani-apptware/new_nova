...
// import Offering, { IOffering } from "../model/Portfolio";
...
export async function uploadFileToDataRoom(
  ...
  try {
    ...
    newFileFormData.append(
      "description",
      `Image for the listing name: ${listing?.name}`
    );
    ...
    newFileFormData.append(
      "ari",
      `ari:drs:us:client:agsiri:dataroom:${createDataRoomName(listing?.name)}`
    );
    newFileFormData.append("dataroom_id", `${listing?.dataroom_id}`);

    // Make the axios POST request
    const response = await axios.post(
      `${config.dataRoomServiceUrl}/files`,
      newFileFormData
    );
