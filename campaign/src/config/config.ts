...
interface Config {
  ...
  // The URL for the listing service
  listingServiceUrl?: string;

  // The URL for the offering service
...
const config: Config = {
  ...
  // default local URL
  dataRoomServiceUrl: process.env.DATA_ROOM_SERVICE_URL || 'http://localhost:3001',
  ...
  // Set the URL for the listing service
  listingServiceUrl: process.env.LISTING_SERVICE_URL || "http://localhost:6000",

  // Set the URL for the offering service
  offeringServiceUrl: process.env.OFFERING_SERVICE_URL || "http://localhost:9000",
};
