...
export async function uploadFileToDataRoom(
  ...
  try {
    ...
    const ari = `${config.ari.prefix}:${config.ari.namespace}:${config.ari.service}:${config.ari.region}:${config.ari.accountId}:${config.ari.resource}/${createDataRoomName(
      campaign.name
    )}`
    ...
    newFileFormData.append("description", `Image for the campaign name: ${campaign.name}`);
    ...
    newFileFormData.append("ari", ari);
    newFileFormData.append("dataroom_id", dataroomId);
...
export async function getOfferAndListingData(offeringId: mongoose.Types.ObjectId, authHeader: string) {

  const offer = await axios
    .get(
      `${config.offeringServiceUrl}/api/offerings/${offeringId}`,
      {
        headers: {
    ...
    .catch((error) => {
      ...
      throw new Error(translate("ERROR_FETCH_OFFERING"));
    });
